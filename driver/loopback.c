...
// Struct to hold the queue exclusively for loopback.
struct loopback {
    int irq;
    mutex_t mutex;
...
// Queue to hold the data to be transmitted.
// The following transmit function enqueues the data, and the interrupt handler dequeues. The data is passed to the input function of the
...
static int loopback_transmit(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst) {
    ...
    // NOTE: data[] is flexible array member, so sizeof() returns the size of the struct without the flexible array member.
    // sizeof(*entry) == sizeof(type) + sizeof(len).
...
// Initialize a loopback network device and return the registered loopback device.
