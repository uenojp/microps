...
// Get a hardware address of the device.
static int ether_tap_addr(struct net_device *dev) {
    int soc;
    struct ifreq ifr = {};

    // Create a socket since ioctl()'s SIOCGIFHWADDR request is only valid for opened fd.
...
// Open and register TUN/TAP device.
...
static int ether_tap_isr(unsigned int irq, void *id) {
    ...
    pfd.events = POLLIN;
    while (1) {
        // poll(2) so that it does not block in the interrupt handler.
        ret = poll(&pfd, 1, 0);
        if (ret == -1) {
            if (errno == EINTR) {
                continue;
...
// Initialize a TUN/TAP device and return the registered the device.
struct net_device *ether_tap_init(const char *name, const char *addr) {
    ...
    if (addr) {
        if (ether_addr_pton(addr, (uint8_t *)dev->addr) == -1) {
